...
class MqttPublisher:
    ...
    def publish_monthly_revenue_history(self, monthly_data: list[dict]) -> None:
        """Publish monthly revenue history as JSON attributes for charting."""
        payload = json.dumps({"months": monthly_data})
        self._publish(
            f"{self.prefix}/revenue/monthly_history",
            payload,
            retain=True,
        )
        self._publish(
            f"{self.prefix}/revenue/monthly_history/state",
            str(len(monthly_data)),
            retain=True,
        )
    ...
    def _publish_discovery(self) -> None:
        ...
        sensors = [
            ...
            ("today_battery_charge_kwh", "revenue/today/battery_charge_kwh", "Today Battery Charge", "kWh", "mdi:battery-charging"),
            ("today_charge_cost", "revenue/today/charge_cost_pence", "Today Charge Cost", "p", "mdi:cash-remove"),
            ("today_arbitrage_profit", "revenue/today/arbitrage_profit_pence", "Today Arbitrage Profit", "p", "mdi:cash-check"),
        ...
        schedule_sensors = [
            ...
            ("monthly_revenue_history", "revenue/monthly_history", "Monthly Revenue History", "mdi:chart-bar"),
        ]
