...
class TestFlatBaseline:
    """Flat baseline always uses actual meter kWh at flat rate."""

    def test_flat_uses_meter_kwh(self, calculator):
        """Flat baseline uses same kWh as actual export."""
        meter = make_meter_interval(kwh=5.0)
        tariff = make_tariff_slot(
            interval_start=meter.interval_start, rate_pence=20.0
        )
        ...
        assert result.export_kwh == 5.0
        assert result.agile_revenue_pence == 100.0  # 5 × 20p
        assert result.flat_export_kwh == 5.0  # Same as actual
        assert result.flat_revenue_pence == 60.0  # 5.0 × 12p
        assert result.uplift_pence == 40.0  # 100 - 60

    def test_batch_flat_uses_meter_kwh(self, calculator):
        """calculate_batch flat baseline uses meter kWh for each interval."""
        start1 = datetime(2026, 3, 9, 12, 0, tzinfo=timezone.utc)
        start2 = datetime(2026, 3, 9, 12, 30, tzinfo=timezone.utc)
        ...
        assert len(results) == 2
        assert results[0].flat_export_kwh == 3.0
        assert results[0].flat_revenue_pence == 36.0  # 3.0 × 12p
        assert results[1].flat_export_kwh == 4.0
        assert results[1].flat_revenue_pence == 48.0  # 4.0 × 12p

    def test_zero_export_zero_flat(self, calculator):
        """Zero export → zero flat revenue."""
        meter = make_meter_interval(kwh=0.0)
        tariff = make_tariff_slot(
            interval_start=meter.interval_start, rate_pence=20.0
        )
